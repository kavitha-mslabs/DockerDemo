...
@Named
@ViewScoped
public class ComplaintResentReport implements Serializable {
	...
	@PostConstruct
	public void init() {
		System.out.println("Initializing COMPLAINT RESENT REPORT...");
	...
	@Transactional
	public void getComplaintResentDetail() {
	    try (Session session = sessionFactory.openSession()) {
	        ...
	        String hql = "SELECT " +
	        	    "a.id, " +
	        	    "TO_CHAR(a.created_on, 'dd-mm-yyyy-hh24:mi') AS ccdt, " +
	        	    "DECODE(a.device, " +
	        	        "'web', 'Web', " +
	        	        "'FOC', 'FOC', " +
	        	        "'admin', 'FOC', " +
	        	    ...
	        	    "a.service_name, " + 
	        	    "a.service_address, " +
	        	    "b.mobile AS contnumber, " + 
	        	    "k.name AS ctyp, " +
	        	    "a.description, " + 
	        	    "DECODE(a.status_id, 0, 'Pending', 1, 'In Progress', 2, 'Completed') AS cstat, " +
	                "(CASE WHEN a.status_id=2 then TO_CHAR(a.updated_on, 'DD-MM-YYYY-HH24:MI') else '' end) AS cupdt, " +
	                "(CASE WHEN a.status_id=2 then c.description else '' end) AS attremarks, " +
	        	    "f.name AS regname, " +
	        	    "g.name AS cirname, " +
	        	    "h.name AS divname, " +
	        	    "i.name AS sdivname, " +
	        	    "j.name AS secname, " +
	        	    "a.REGION_ID AS regID, " +
	        	    "a.CIRCLE_ID AS cirID, " +
	        	    "a.DIVISION_ID AS divID, " +
	        	    "a.SUB_DIVISION_ID AS sdivID, " +
	        	    "a.SECTION_ID AS secID, " +
	        	    "l.remarks AS resrem, " +
	        	    "TO_CHAR(l.RESEND_ON, 'dd-mm-yyyy-hh24:mi') AS resdt, " +
	        	    "l.ENTRYUSER AS resuser " +
	        	"FROM " +
	        	    "COMPLAINT a " +
	        	    ...
	        	    "JOIN COMP_RESEND l ON l.comp_id = a.id " +
	        	"WHERE " +
	        	    "a.REGION_ID IS NOT NULL " + 
	        	    "AND a.circle_id <> 0 " + 
	        	    "AND a.division_id <> 0 " + 
	        	    "AND a.sub_division_id <> 0 " + 
	        	    "AND a.section_id <> 0 " + 
	        	    "AND TRUNC(l.RESEND_ON) >= TO_DATE(:fromDate, 'DD-MM-YYYY') " + 
	        	    "AND TRUNC(l.RESEND_ON) <= TO_DATE(:toDate, 'DD-MM-YYYY') " +
	        ...
	        for (Object[] row : results) {
	        	...
	        	dto.setRegionId(row[17] != null ? ((BigDecimal) row[17]).intValue() : null);
	        	dto.setCircleId(row[18] != null ? ((BigDecimal) row[18]).intValue() : null);
	        	dto.setDivisionId(row[19] != null ? ((BigDecimal) row[19]).intValue() : null);
	        	dto.setSubDivisionId(row[20] != null ? ((BigDecimal) row[20]).intValue() : null);
	        	dto.setSectionId(row[21] != null ? ((BigDecimal) row[21]).intValue() : null);
	        	dto.setResentRemarks((String) row[22]);
	        	dto.setResentDate((String) row[23]);
	        	dto.setResentBy((String) row[24]);
	      ...
	      //SECTION
	        ...
	        System.out.println("THE COMPLAINT LIST ------------- RESENT-------"+complaintList.size());
	        
	        if(complaintList.size()==0) {
	        	FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR,"","No Complaints Resent For The Given Date");
	...
	public void returnComplaintResentPage() throws IOException {
		FacesContext.getCurrentInstance().getExternalContext().redirect("complaintResentReport.xhtml");
	}
	
	
	public void exportComplaintListToExcel(List<ComplaintDetailValueBean> complaintList) throws IOException {
	    HSSFWorkbook workbook = new HSSFWorkbook();
	    Sheet sheet = workbook.createSheet("Complaint_Resent_Report");
	    ...
	    mainHeadingCell.setCellValue("COMPLAINT RESENT REPORT");
       ...
       // SUB HEADING
	    ...
	    subHeadingCell.setCellValue("FROM: " + formattedFromDate + "      TO " + formattedToDate);
	    ...
	    String[] headers = new String[] {
	        ...
	        "Resent By", "Resent Date", "Resent Remarks", "Region", "Circle", 
	    ...
	    for (ComplaintDetailValueBean complaint : complaintList) {
	        ...
	        row.createCell(2).setCellValue(complaint.getComplaintDate());
	        row.createCell(3).setCellValue(complaint.getComplaintMode());
	        row.createCell(4).setCellValue(complaint.getConsumerNumber());
	        row.createCell(5).setCellValue(complaint.getConsumerDetail());
	        row.createCell(6).setCellValue(complaint.getContactNumber());
	        row.createCell(7).setCellValue(complaint.getComplaintType());
	        row.createCell(8).setCellValue(complaint.getComplaintDescription());
	        row.createCell(9).setCellValue(complaint.getResentBy());
	        row.createCell(10).setCellValue(complaint.getResentDate());
	        row.createCell(11).setCellValue(complaint.getResentRemarks());
	        row.createCell(12).setCellValue(complaint.getRegion());
	        row.createCell(13).setCellValue(complaint.getCircle());
	        row.createCell(14).setCellValue(complaint.getDivision());
	        row.createCell(15).setCellValue(complaint.getSubDivision());
	        row.createCell(16).setCellValue(complaint.getSection());
	    ...
	    try {
	        excelFile = DefaultStreamedContent.builder()
	                .name("Complaint_Resent_Report.xls")
	...
	public void exportComplaintListToPDF(List<ComplaintDetailValueBean> complaintList) {
	    ...
	    try {
	        ...
	        Paragraph title = new Paragraph("COMPLAINT RESENT REPORT", titleFont);
	        ...
	        Paragraph subTitle = new Paragraph("FROM : " + dmFilter.getFromDate() + "  | TO : " + dmFilter.getToDate(), subtitleFont);
	        ...
	        float[] columnWidths = {
	            1.0f, // S.No
	            1.5f, 1.5f, 1.5f, 2f, 3f, 2f, 2f, 3f, 2f, 2f,
	            2f, 1.5f, 1.5f, 1.5f, 2f, 1.5f
	        };
	        table.setWidths(columnWidths);

	        String[] headers = {
	            "S.No", "Complaint ID", "Complaint Date", "Complaint Mode", "Consumer Number", "Consumer Details",
	            "Contact Number", "Complaint Type", "Complaint Description", "Resent By", "Resent Date",
	            "Resent Remarks", "Region", "Circle", "Division", "Sub Division", "Section"
	        };
	        ...
	        if (complaintList != null && !complaintList.isEmpty()) {
	            int serialNo = 1;
	            for (ComplaintDetailValueBean complaint : complaintList) {
	                addCell(table, String.valueOf(serialNo++)); 
	                addCell(table, complaint.getComplaintId() != null ? complaint.getComplaintId().toString() : "");
	                addCell(table, complaint.getComplaintDate());
	                addCell(table, complaint.getComplaintMode());
	                addCell(table, complaint.getConsumerNumber());
	                addCell(table, complaint.getConsumerDetail());
	                addCell(table, complaint.getContactNumber());
	                addCell(table, complaint.getComplaintType());
	                addCell(table, complaint.getComplaintDescription());
	                addCell(table, complaint.getResentBy());
	                addCell(table, complaint.getResentDate());
	                addCell(table, complaint.getResentRemarks());
	                addCell(table, complaint.getRegion());
	                addCell(table, complaint.getCircle());
	                addCell(table, complaint.getDivision());
	                addCell(table, complaint.getSubDivision());
	                addCell(table, complaint.getSection());
	            }
	        } else {
	        ...
	        pdfFile = DefaultStreamedContent.builder()
	            ...
	            .name("Complaint_Resent_Report.pdf")
	...
	// Helper method to add cell text
...
complaint.setResendDetails(resendResultList);
        ...
        String qcHql = "SELECT to_char(QC_ON, 'dd-mm-yyyy-hh24:mi'), QC_STATUS, REMARKS " +
		  ...
		  FacesContext.getCurrentInstance().getExternalContext().redirect("complaintResentDetail.xhtml");
		  
		  System.out.println("THE SELECTED COMP-------------"+complaint.getComplaintId());
