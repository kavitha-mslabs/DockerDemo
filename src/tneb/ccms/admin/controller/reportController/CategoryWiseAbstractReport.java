...
@Named
@ViewScoped
public class CategoryWiseAbstractReport implements Serializable {
	...
	@PostConstruct
	public void init() {
		System.out.println("INITIALIZED CATEGORY WISE COMPLAINTS - ABSTRACT REPORT");
		...
		if (roleId >= 1 && roleId <= 3) {	
			...
			searchCategoryWiseComplaintsAbstractReport();  // CATEGORY WISE COMPLAINTS ABSTRACT
		}
		}else {
			searchCategoryWiseComplaintsAbstractReport(); 
		}
	...
	@Transactional
	public void searchCategoryWiseComplaintsAbstractReport() {
		...
		try {
		    ...
		    if(callCenterValueBean!=null && (callCenterValueBean.getRoleId()==1 || callCenterValueBean.getRoleId()==7)) {
		    	...
		    	System.err.println("THE REGON CODE---------------"+dmFilter.getRegionCode());
		    	System.err.println("THE CIRCLE CODE---------------"+dmFilter.getCircleCode());
		    	System.err.println("THE circle list---------------"+circleList.size());
		    	...
		    	System.err.println("THE circle list after---------------"+circleList.size());
			...
			if(dmFilter.getRegionCode().equals("A")) {
				if(dmFilter.getCircleCode().equals("A")) {
					...
					reports = circleList.stream().filter(c ->c.getCircode().equals(dmFilter.getCircleCode())).collect(Collectors.toList());
				}
			}else {
				if(dmFilter.getCircleCode().equals("A")) {
					...
					reports = circleList.stream().filter(a -> dmFilter.getRegionCode().equals(a.getRegcode())).filter(c->dmFilter.getCircleCode().contains(c.getCircode())).collect(Collectors.toList());
	...
	public void fetchReportForSectionUsers(String circleCode,String circleName) {
		...
		try {
			...
			String hql = "SELECT c.*,d.NAME as DIVISION_NAME,d.id as DIVISION_ID,s.SUB_DIVISION_ID as SUB_DIVISION FROM ABST_SEC_CT c,SECTION s,DIVISION d WHERE c.SECCODE = s.ID AND d.ID =s.DIVISION_ID AND c.CIRCODE= :circleCode ";
			...
			if(adminUserValueBean.getRoleId()==1) {
				sectionList = sectionList.stream().filter(section -> section.getSecCode().equals(adminUserValueBean.getSectionId().toString())).collect(Collectors.toList());
			}
			
			FacesContext.getCurrentInstance().getExternalContext().redirect("categoryWiseSectionAbstractReport.xhtml");
...
//			}
	...
	public void redirectToCircleReport() throws IOException {
		FacesContext.getCurrentInstance().getExternalContext().redirect("categoryWiseCircleAbstarctReport.xhtml");	
	}
	...
	public void getComplaintListForSection(String secCode,String sectionName) throws IOException {
		...
		String hql = "SELECT a.id, "+
                ...
                //"LEFT JOIN DISTRIB_MASTER dm ON dm.reg_no = f.ID AND dm.SCODE = j.code AND dm.DISTRIB_CODE =ct.STCODE "+
...
if (callCenterValueBean != null) {
	            ...
	            if(roleId==1) {
	            	hql+=" AND a.circle_id IN (:circleIds)";
	            }
	            if(roleId==5) {
	            ...
	            if (roleId >= 6 && roleId <= 9) {
	                ...
	                hql+=" AND a.region_id = :regionId AND a.circle_id = :circleId";
	            } 
...
complaint.setResendDetails(resendResultList);
	...
	public void exportToExcel(List<TmpCtCirBean> reports) throws IOException {
	    ...
	    Sheet sheet = workbook.createSheet("CategoryWise_Complaints_Abstract_Report-CircleWise");
	    ...
	    headingCell.setCellValue("CATEGORY WISE COMPLAINTS ABSTRACT REPORT - CIRCLE WISE");
	    ...
	    try {
	        excelFile = DefaultStreamedContent.builder()
	                .name("CategoryWise_Complaints_Abstract_Report-CircleWise.xls")
	...
	//SECTION REPORT TO EXCEL DOWNLOAD
	 public void exportSectionsToExcel(List<TmpCtSecBean> reports) throws IOException {
		    HSSFWorkbook workbook = new HSSFWorkbook();
		    Sheet sheet = workbook.createSheet("CategoryWise_Complaints_Abstract_Report-SectionWise");

	        sheet.setColumnWidth(0, 6000); // SECTION
	        sheet.setColumnWidth(1, 6000); // DIVISION
		    ...
		    Row headingRow = sheet.createRow(0);
		    Cell headingCell = headingRow.createCell(0);
		    headingCell.setCellValue("CATEGORY WISE COMPLAINTS ABSTRACT REPORT - SECTION WISE");
		    headingCell.setCellStyle(headingStyle);
		    sheet.addMergedRegion(new CellRangeAddress(0, 0, 0, 31)); 
		    ...
		    try {
		        excelFile = DefaultStreamedContent.builder()
		                .name("CategoryWise_Complaints_Abstract_Report-SectionWise.xls")
		                .contentType("application/vnd.ms-excel")
		                .stream(() -> inputStream)
		                .build();
		    } catch (Exception e) {
	 ...
	 public void exportToPdf(List<TmpCtCirBean> circleList) throws IOException {
			    ...
			    final float FIRST_COLUMN_WIDTH = 4f; //SNO
			    final float SECOND_COLUMN_WIDTH = 15f; //CIRCLE
			    ...
			    try {
			        ...
			        Paragraph title = new Paragraph("CATEGORY WISE COMPLAINTS ABSTRACT REPORT - CIRCLE WISE", TITLE_FONT);
			        ...
			        columnWidths[1] = SECOND_COLUMN_WIDTH; 
			        for (int i = 2; i < COLUMN_COUNT; i++) {
			            columnWidths[i] = OTHER_COLUMN_WIDTH;
			        }
			        table.setWidths(columnWidths);
			        addMergedHeaderCell(table, "S.No", HEADER_FONT, 1, 1);
			        addMergedHeaderCell(table, "CIRCLE", HEADER_FONT, 1, 1);
			        ...
			        for (String header : categoryHeaders) {
			            addMergedHeaderCell(table, header, HEADER_FONT, 3, 1);
			        }
			        addMergedHeaderCell(table, "TOTAL", HEADER_FONT, 3, 1);
			        
			        PdfPCell emptyCell = new PdfPCell(new Phrase("", HEADER_FONT));
			        emptyCell.setBackgroundColor(BaseColor.LIGHT_GRAY);
			        table.addCell(emptyCell);
			        
			      //SUB HEADER
			        String[] subHeaders = {"Revd.", "Comp.", "Pend."};
			        emptyCell = new PdfPCell(new Phrase("", HEADER_FONT));
			        emptyCell.setBackgroundColor(BaseColor.LIGHT_GRAY);
			        table.addCell(emptyCell);
			        ...
			        PdfPCell footerLabel1 = new PdfPCell(new Phrase("", TOTAL_FONT)); // Empty cell for S.No
			        ...
			        pdfFile = DefaultStreamedContent.builder()
				            ...
				            .name("CategoryWise_Complaints_Abstract_Report-CircleWise.pdf")
		 ...
		 public void exportSectionsToPdf(List<TmpCtSecBean> sectionList) throws IOException {
			    ...
			    final float FIRST_COLUMN_WIDTH = 4f;
			    final float SECOND_COLUMN_WIDTH = 15f;
			    final float OTHER_COLUMN_WIDTH = 4f;
			    final Font HEADER_FONT = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10, BaseColor.BLACK);
			    final Font TOTAL_FONT = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10, BaseColor.BLACK);
			    final Font DATA_FONT = FontFactory.getFont(FontFactory.HELVETICA, 10);
			    ...
			    try {
			        ...
			        Paragraph title = new Paragraph("CATEGORY WISE COMPLAINTS ABSTRACT REPORT - SECTION WISE", TITLE_FONT);
			        title.setAlignment(Element.ALIGN_CENTER);
			        document.add(title);
			        
			        Paragraph subTitle = new Paragraph("CIRCLE :"+selectedCircleName);
			        ...
			        columnWidths[0] = FIRST_COLUMN_WIDTH;
			        columnWidths[1] = SECOND_COLUMN_WIDTH;
			        columnWidths[2] = SECOND_COLUMN_WIDTH;
			        ...
			        addMergedHeaderCell(table, "SECTION", HEADER_FONT, 1, 1);
			        addMergedHeaderCell(table, "DIVISION", HEADER_FONT, 1, 1);
			        String[] categoryHeaders = {"Billing Releated","Meter Releated","Power Failure","Voltage Fluctuation","Fire","Dangerous Pole","Theft","Conductor Snapping","Others"};
			      ...
			      //SUB HEADER
			        ...
			        pdfFile = DefaultStreamedContent.builder()
				            .contentType("application/pdf")
				            .name("CategoryWise_Complaints_Abstract_Report-SectionWise.pdf")
				            .stream(() -> new ByteArrayInputStream(outputStream.toByteArray()))
				            .build();
		  ...
		  private void addMergedHeaderCell(PdfPTable table, String text, Font font, int colSpan, int rowSpan) {
			...
			public void exportComplaintListToExcel(List<ViewComplaintReportValueBean> complaintList) throws IOException {
			    HSSFWorkbook workbook = new HSSFWorkbook();
			    Sheet sheet = workbook.createSheet("CategoryWise_Complaints_Abstract_Report-ComplaintList");
			    ...
			    titleCell.setCellValue("CATEGORY WISE ABSTRACT REPORT COMPLAINT LIST");
			    titleCell.setCellStyle(headingStyle);
			    sheet.addMergedRegion(new CellRangeAddress(0, 0, 0, 16));
			    
			    Row subtitleRow = sheet.createRow(1);
			    Cell subtitleCell = subtitleRow.createCell(0);
			    subtitleCell.setCellValue("Circle: " + selectedCircleName + "  |  Section: " + selectedSectionName);
			    ...
			    try {
			        excelFile = DefaultStreamedContent.builder()
			                .name("CategoryWise_Complaints_Abstract_Report-ComplaintList.xls")
			                .contentType("application/vnd.ms-excel")
			                .stream(() -> inputStream)
			                .build();
			    } catch (Exception e) {
			...
			public void exportComplaintListToPdf(List<ViewComplaintReportValueBean> complaintList) {
			    ...
			    try {
			        ...
			        Paragraph title = new Paragraph("CATEGORY WISE ABSTRACT REPORT COMPLAINT LIST", titleFont);
			        title.setAlignment(Element.ALIGN_CENTER);
			        ...
			        pdfFile = DefaultStreamedContent.builder()
			            .contentType("application/pdf")
			            .name("Category_Wise_Complaints_Abstract_Report.pdf")
			            .stream(() -> new ByteArrayInputStream(outputStream.toByteArray()))
			            .build();
