...
@Named
@ViewScoped
public class TopOfficesAbstractReport implements Serializable {
	...
	@PostConstruct
	public void init() {
		System.out.println("Initializing TopOfficesAbstractReport...");
	...
	public void search() {
       ...
       if (dmFilter.getFromDate().after(dmFilter.getToDate())) {
		...
		try {
			...
			fetchReports(session,dmFilter.getNoOfOffices());
		} catch (Exception e) {
			e.printStackTrace();
	...
	@Transactional
	private void fetchReports(Session session, Integer noOfOffices) {
		try {
			...
			String hql = "SELECT REGCODE, REGNAME, CIRCODE, CIRNAME, " +
                    "BLTOT, BLCOM, BLPEN, METOT, MECOM, MEPEN, PFTOT, PFCOM, PFPEN, " +
                    "VFTOT, VFCOM, VFPEN, FITOT, FICOM, FIPEN, THTOT, THCOM, THPEN, " +
                    "TETOT, TECOM, TEPEN, CSTOT, CSCOM, CSPEN, OTTOT, OTCOM, OTPEN, " +
                    "(COALESCE(BLPEN, 0) + COALESCE(MEPEN, 0) + COALESCE(PFPEN, 0) + " +
                    " COALESCE(VFPEN, 0) + COALESCE(FIPEN, 0) + COALESCE(THPEN, 0) + " +
                    " COALESCE(TEPEN, 0) + COALESCE(CSPEN, 0) + COALESCE(OTPEN, 0)) AS TOTAL_PEN " +
                    "FROM TMP_CT_CIR " +
			...
			List<TmpCtCirBean> result = new ArrayList<TmpCtCirBean>();
			reports = new ArrayList<>();
			...
			if(dmFilter.getRegionCode().equals("A")) {
				...
				if(dmFilter.getCircleCode().equals("A")) {
					reports = result.stream().filter(r ->r.getRegcode().equals(dmFilter.getRegionCode())).collect(Collectors.toList());
				}else {
					reports = result.stream().filter(r ->r.getRegcode().equals(dmFilter.getRegionCode()))
	...
	public void fetchReportByCircle(String circleCode , String circleName) {
		...
		try {
			...
			System.out.println("THE FROM DATE FOR SECTION REPORT =*******"+formattedFromDate);
			System.out.println("THE TO DATE FOR SECTION REPORT =*******"+formattedToDate);
			...
			session.createNativeQuery(
							"BEGIN CAT_SEC_ABST_SINGLE_ALL; END;")
							.executeUpdate();
			...
			String hql = "SELECT c.*,d.NAME,"+
					"(COALESCE(BLPEN, 0) + COALESCE(MEPEN, 0) + COALESCE(PFPEN, 0) + " +
                    " COALESCE(VFPEN, 0) + COALESCE(FIPEN, 0) + COALESCE(THPEN, 0) + " +
                    " COALESCE(TEPEN, 0) + COALESCE(CSPEN, 0) + COALESCE(OTPEN, 0)) AS TOTAL_PEN " +
                    "FROM TMP_CT_SEC c, SECTION s, DIVISION d " +
                    "WHERE c.SECCODE = s.ID AND d.ID = s.DIVISION_ID " +
                    "ORDER BY TOTAL_PEN DESC";
			
			@SuppressWarnings("unchecked")
			List<Object[]> results = session.createNativeQuery(hql).setMaxResults(10).getResultList();
	        ...
	        System.err.println("THE SIZE OF SECTION LIST ======"+sectionList.size());
	        System.err.println("THE CIRCLE CODE ======"+circleCode);


            FacesContext.getCurrentInstance().getExternalContext().redirect("topOfficesSectionAbstract.xhtml");
	...
	public void fetchReportForSectionUsers() {
		...
		try {
			...
			String hql = "SELECT c.*,d.NAME,d.id as DivisionId,s.sub_division_id as SubDivision, "+
					"(COALESCE(BLPEN, 0) + COALESCE(MEPEN, 0) + COALESCE(PFPEN, 0) + " +
                    " COALESCE(VFPEN, 0) + COALESCE(FIPEN, 0) + COALESCE(THPEN, 0) + " +
                    " COALESCE(TEPEN, 0) + COALESCE(CSPEN, 0) + COALESCE(OTPEN, 0)) AS TOTAL_PEN " +
                    "FROM TMP_CT_SEC c, SECTION s, DIVISION d " +
			...
			List<TmpCtSecBean> result = new ArrayList<TmpCtSecBean>();
	        sectionList = new ArrayList<>();
	      ...
	      //SECTION
			...
			if (dmFilter.getNoOfOffices() != null && dmFilter.getNoOfOffices() > 0) {
			    sectionList = sectionList.stream()
			                    .limit(dmFilter.getNoOfOffices())
			                    .collect(Collectors.toList());
			}
...
//            dto.setQcRemarks((String) row[28]);        	
	...
	public void exportToExcel(List<TmpCtCirBean> reports) throws IOException {
	    ...
	    Sheet sheet = workbook.createSheet("Pending_Top_Circle_Offices_Abstract_Report");

	    // Set column widths
	    sheet.setColumnWidth(0, 2500);  // Increased width for S.NO column
	    sheet.setColumnWidth(1, 5000);  // Increased width for CIRCLE column
	    for (int i = 2; i < 31; i++) {  
	    ...
	    // MAIN HEADING STYLE - Enhanced with borders and better contrast
	    ...
	    mainHeadingCell.setCellValue("PENDING TOP CIRCLE OFFICES ABSTRACT REPORT");
	    ...
	    // Create sub-heading with date and device info
	    Row subHeadingRow = sheet.createRow(1);
	    subHeadingRow.setHeightInPoints(20);  // Increased row height
	    ...
	    // HEADER STYLES - Enhanced with different colors for main and sub headers
	    ...
	    // Create headers with increased height
	    Row headerRow1 = sheet.createRow(2);
	    headerRow1.setHeightInPoints(20);  // Increased row height
	    Row headerRow2 = sheet.createRow(3);
	    headerRow2.setHeightInPoints(20);  // Increased row height

	    String[] mainHeaders = {"S.NO", "CIRCLE", "Billing Related", "Meter Related", "Power Failure", 
	                          "Voltage Fluctuation", "Fire", "Dangerous Pole", "Theft", 
	                          "Conductor Snapping", "Others", "TOTAL"};
	    ...
	    for (String mainHeader : mainHeaders) {
	        ...
	        if (!mainHeader.equals("S.NO") && !mainHeader.equals("CIRCLE")) {
	            ...
	            // Merge S.NO and CIRCLE cells vertically
	            sheet.addMergedRegion(new CellRangeAddress(2, 3, colIndex, colIndex));
	            Cell subCell = headerRow2.createCell(colIndex);
	            subCell.setCellStyle(mainHeaderStyle);
	            colIndex++;
	        }
	    }
	    ...
	    CellStyle circleNameStyle = workbook.createCellStyle();
	    circleNameStyle.setAlignment(HorizontalAlignment.LEFT);
	    circleNameStyle.setBorderBottom(BorderStyle.THIN);
	    circleNameStyle.setBorderTop(BorderStyle.THIN);
	    circleNameStyle.setBorderLeft(BorderStyle.THIN);
	    circleNameStyle.setBorderRight(BorderStyle.THIN);
	    circleNameStyle.setIndention((short) 1);  // Add some indentation
	    ...
	    for (TmpCtCirBean report : reports) {
	        ...
	        // S.NO cell with styling
	        Cell snoCell = row.createCell(0);
	        snoCell.setCellValue(serialNumber++);
	        snoCell.setCellStyle(dataCellStyle);
	        
	        // CIRCLE cell with left alignment
	        Cell circleCell = row.createCell(1);
	        circleCell.setCellValue(report.getCirname());
	        circleCell.setCellStyle(circleNameStyle);
	        ...
	        for (int i = 0; i < values.length; i += 3) {
	            setCellValueWithStyle(row, dataIndex++, values[i], dataCellStyle);
	            setCellValueWithStyle(row, dataIndex++, values[i + 1], dataCellStyle);
	            setCellValueWithStyle(row, dataIndex++, values[i + 2], dataCellStyle);
	        ...
	        setCellValueWithStyle(row, dataIndex++, rowTotSum, dataCellStyle);
	        setCellValueWithStyle(row, dataIndex++, rowCompSum, dataCellStyle);
	        setCellValueWithStyle(row, dataIndex, rowPendSum, dataCellStyle);
	    ...
	    totalFont.setColor(IndexedColors.BLACK.getIndex());  // White font
	    ...
	    totalRowStyle.setFillForegroundColor(IndexedColors.GREY_25_PERCENT.getIndex());  // Dark background
	    ...
	    totalRow.setHeightInPoints(25);  // Increased row height for total row
	    
	    Cell totalLabelCell1 = totalRow.createCell(0);
	    totalLabelCell1.setCellValue("");
	    totalLabelCell1.setCellStyle(totalRowStyle);
	    
	    Cell totalLabelCell2 = totalRow.createCell(1);
	    totalLabelCell2.setCellValue("TOTAL");
	    totalLabelCell2.setCellStyle(totalRowStyle);

	    int totalDataIndex = 2;
	    for (int i = 0; i < totalSums.length; i++) {
	        setCellValueWithStyle(totalRow, totalDataIndex++, totalSums[i], totalRowStyle);
	    }
	    ...
	    try {
	        excelFile = DefaultStreamedContent.builder()
	                .name("Pending_Top_Circle_Offices_Abstract_Report.xls")
	...
	private void setCellValueWithStyle(Row row, int index, BigDecimal value, CellStyle style) {
	...
	private void setCellValue(Row row, int index, BigDecimal value) {
	    Cell cell = row.createCell(index);
	    if (value != null) {
	        cell.setCellValue(value.doubleValue());
	    } else {
	 ...
	 public void exportSectionsToExcel(List<TmpCtSecBean> reports) throws IOException {
	  ...
	  public void exportToPdf(List<TmpCtCirBean> circleList) throws IOException {
		    ...
		    try {
		        ...
		        Paragraph title = new Paragraph("PENDING TOP CIRCLE OFFICE ABSTRACT REPORT", TITLE_FONT);
		      ...
		      //SUB HEADER
		        ...
		        pdfFile = DefaultStreamedContent.builder()
			            ...
			            .name("Pending_Top_Circle_Offices_Abstract_Report.pdf")
	  ...
	  public void exportSectionsToPdf(List<TmpCtSecBean> sectionList) throws IOException {
		    final int CATEGORY = 9;
		    final int SUB_COLUMNS = 3;
		    final int COLUMN_COUNT = 1+2 + (CATEGORY * SUB_COLUMNS) + 3;  // S.NO+ SECTION + DIVISION + 9*3 +3
		    ...
		    try {
		        ...
		        Paragraph subTitle = new Paragraph("FROM : " + fromDate +","+ " TO " +toDate +","+" Device: "+device+","+"Circle :"+selectedCircleName);
		        subTitle.setAlignment(Element.ALIGN_CENTER);
		        subTitle.setSpacingAfter(10);
		        document.add(subTitle);
