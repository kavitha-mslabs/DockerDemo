...
@Named
@ViewScoped
public class ContactNumberHistoryReport implements Serializable {

	private static final long serialVersionUID = 1L;
	List<ViewComplaintReportValueBean> complaintList ;
	List<ViewComplaintReportValueBean> contactComplaintHistory;
	...
	@PostConstruct
	public void init() {
		System.out.println("Initializing CONTACT NUMBER BASED HISTORY REPORT...");
	...
	public void resetIfNeeded() {
        if (!FacesContext.getCurrentInstance().isPostback() && !cameFromInsideReport) {
        	...
        	dmFilter.setContactNumber(null);
        }
        cameFromInsideReport = false;
    }

	@Transactional
	public void getContactNumberBasedHistory() {
	    FacesContext context = FacesContext.getCurrentInstance();
	    String contactNumber = dmFilter.getContactNumber();
	    ...
	    if (!contactNumber.matches("\\d{10}")) {
	    	FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR,
                    "Error", "Contact Number Must Be in 10 Digits");
	    	context.addMessage(null, message);
	        return;
	    }
	    
	    try (Session session = sessionFactory.openSession()) {
	        // Get user session information first
	        ...
	        // Base query with contact number filtering
	        StringBuilder sql = new StringBuilder(
	            ...
	            "    WHERE (" +
	            "        EXISTS (" +
	            "            SELECT 1 FROM COMP_CONTACT_MAP ccm2 " +
	            "            WHERE ccm2.COMP_ID = a.id AND ccm2.CONTACTNO = :contactNumber" +
	            "        ) OR (" +
	            "            NOT EXISTS (" +
	            "                SELECT 1 FROM COMP_CONTACT_MAP ccm2 " +
	            "                WHERE ccm2.COMP_ID = a.id AND ccm2.CONTACTNO = :contactNumber" +
	            "            ) AND b.mobile = :contactNumber" +
	            "        )" +
	        ...
	        if (callCenterValueBean != null) {
	            ...
	            if(roleId==1) {
	            	sql.append(" AND a.circle_id IN (:circleIds)");
	            }
	            if(roleId==5) {
	            	sql.append(" AND a.device =:device");
	            }
	            if(roleId==7 ) {
	            	sql.append(" AND a.circle_id IN (:circleIds)");
	            	sql.append(" AND a.device =:device");
	            ...
	            if (roleId >= 6 && roleId <= 9) {
	                ...
	                sql.append(" AND a.region_id = :regionId");
	            } 
	            else if (roleId == 4) { // CIRCLE
	                sql.append(" AND a.region_id = :regionId AND a.circle_id = :circleId");
	            } 
	        ...
	        sql.append(") " +
	            ...
	            "       g.id AS CircleId, " +
	            "       h.id AS DivisionId, " +
	            "       i.id AS SubDivisionId, " +
	            "       j.id AS SectionId " +
	            "FROM   COMPLAINT a " +
	            "JOIN filtered_complaints fc ON a.id = fc.id " +
	            "LEFT JOIN PUBLIC_USER b ON a.user_id = b.id " +
	        ...
	        query.setParameter("contactNumber", dmFilter.getContactNumber());
	        ...
	        if(complaintList.isEmpty()) {
	            FacesContext.getCurrentInstance().addMessage(null, 
	                ...
	                "No Data", "No Complaints For The Given Contact Number And Given Date"));
	            return;
	        }
	    } catch (Exception e) {
	        e.printStackTrace();
	...
	public void redirectToContactHistoryData(String contactNumber) throws IOException {
		 try (Session session = sessionFactory.openSession()) {
	         ...
	         String hql = "SELECT a.id as COMPLAINT_ID, "+
		                ...
		                "g.name AS CIRCLE, "+
		                "h.name AS DIVISION, "+
		                "i.name AS SUB_DIVISION, "+
		                "j.name AS SECTION " +
		                "FROM COMPLAINT a " +
		                "JOIN COMP_CONTACT_MAP cm ON a.id=cm.COMP_ID "+
		                ...
		                "JOIN SUB_DIVISION i ON a.sub_division_id = i.id " +
		                "JOIN SECTION j ON a.section_id = j.id " +
		                "WHERE cm.CONTACTNO =:contactNumber "+
		                "AND a.created_on BETWEEN :formattedFromDate AND :formattedToDate";
		...
		FacesContext.getCurrentInstance().getExternalContext().redirect("contactNumberBasedHistory.xhtml");
	}
	
	//EXCEL DOWNLOAD
	public void exportComplaintListToExcel(List<ViewComplaintReportValueBean> complaintList) throws IOException {
	    HSSFWorkbook workbook = new HSSFWorkbook();
	    Sheet sheet = workbook.createSheet("Contact_Number_Based_History_Report");
	    ...
	    headingCell.setCellValue("CONTACT NUMBER HISTORY REPORT");
	    ...
	    for (ViewComplaintReportValueBean complaint : complaintList) {
	        ...
	        row.getCell(1).setCellValue(complaint.getComplaintId() != null ? complaint.getComplaintId().toString() : "");
	        row.getCell(2).setCellValue(complaint.getCreatedOnFormatted() != null ? complaint.getCreatedOnFormatted() : "");
	        row.getCell(3).setCellValue(complaint.getDevice() != null ? complaint.getDevice() : "");
	        row.getCell(4).setCellValue(complaint.getServiceNumber() != null ? complaint.getServiceNumber() : "");
	        row.getCell(5).setCellValue(complaint.getServiceAddress() != null ? complaint.getServiceAddress() : "");
	        row.getCell(6).setCellValue(complaint.getMobile() != null ? complaint.getMobile() : "");
	        row.getCell(7).setCellValue(complaint.getComplaintType() != null ? complaint.getComplaintType() : "");
	        row.getCell(8).setCellValue(complaint.getComplaintDescription() != null ? complaint.getComplaintDescription() : "");
	        row.getCell(9).setCellValue(complaint.getComplaintStatusValue() != null ? complaint.getComplaintStatusValue() : "");
	        row.getCell(10).setCellValue(complaint.getAttendedDate() != null ? complaint.getAttendedDate() : "");
	        row.getCell(11).setCellValue(complaint.getAttendedRemarks() != null ? complaint.getAttendedRemarks() : "");
	        row.getCell(12).setCellValue(complaint.getRegionName() != null ? complaint.getRegionName() : "");
	        row.getCell(13).setCellValue(complaint.getCircleName() != null ? complaint.getCircleName() : "");
	        row.getCell(14).setCellValue(complaint.getDivisionName() != null ? complaint.getDivisionName() : "");
	        row.getCell(15).setCellValue(complaint.getSubDivisionName() != null ? complaint.getSubDivisionName() : "");
	        row.getCell(16).setCellValue(complaint.getSectionName() != null ? complaint.getSectionName() : "");
	    }
	    ...
	    excelFile = DefaultStreamedContent.builder()
	            .name("Contact_Number_Based_History_Report.xls")
	...
	public void exportComplaintListToPDF(List<ViewComplaintReportValueBean> complaintList) {
	    ...
	    try {
	        ...
	        Paragraph title = new Paragraph("CONTACT NUMBER BASED HISTORY REPORT", titleFont);
	        ...
	        Paragraph subTitle = new Paragraph("CONTACT NUMBER : "+ dmFilter.getContactNumber() + " | "+"FROM : " + fromDateStr + "  | TO : " + toDateStr, subtitleFont);
	        ...
	        float[] columnWidths = {1.5f,1.5f, 1.5f, 1.5f, 2f, 3f, 2f, 2f, 3f, 2f, 2f, 2f, 1.5f, 1.5f, 1.5f, 2f, 1.5f};
	        ...
	        pdfFile = DefaultStreamedContent.builder()
	            ...
	            .name("Contact_Number_Based_History_Report.pdf")
...
complaint.setResendDetails(resendResultList);
        ...
        String qcHql = "SELECT to_char(QC_ON, 'dd-mm-yyyy-hh24:mi'), QC_STATUS, REMARKS " +
		  ...
		  FacesContext.getCurrentInstance().getExternalContext().redirect("contactNumberBasedComplaintDetail.xhtml");
	...
	public List<ViewComplaintReportValueBean> getContactComplaintHistory() {
		return contactComplaintHistory;
	}


	public void setContactComplaintHistory(List<ViewComplaintReportValueBean> contactComplaintHistory) {
		this.contactComplaintHistory = contactComplaintHistory;
	}
