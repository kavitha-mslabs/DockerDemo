...
@Named
@ViewScoped
public class YearWiseComparisonReport {
	...
	@PostConstruct
	public void init() {
		System.out.println("Initializing Year Comparison report...");
	...
	//CIRCLE REPORT 
	@Transactional
	public void searchYearWiseComparisonCircleReport() {
		...
		try {
			...
			if(dmFilter.getYear1()==null) {
				dmFilter.setYear1("2024");
			}
			if(dmFilter.getMonth1()==null) {
				dmFilter.setMonth1("");
			}
			if(dmFilter.getYear2()==null) {
				dmFilter.setYear2("2025");
			}
			if(dmFilter.getMonth2()==null) {
				dmFilter.setMonth2("");
			}
			
			
			int currentYear = Year.now().getValue();
			if (dmFilter.getYear1().equals(dmFilter.getYear2())) {
	            FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR, 
	                "Error", "Year One and Year Two cannot be the same");
	            FacesContext.getCurrentInstance().addMessage(null, message);
	            return;
	        }
			
			if (Integer.parseInt(dmFilter.getYear1()) > currentYear || 
		            Integer.parseInt(dmFilter.getYear2()) > currentYear) {
		            FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR, 
	...
	private void fetchReports(Session session) {
		try {
			...
			List<ComparisonReportValueBean> circleList = new ArrayList<>();
			...
			if(dmFilter.getRegionCode().equals("A")) {
				resultList = circleList;
			}else {
				if(dmFilter.getCircleCode().equals("A")) {
					resultList = circleList.stream().filter(a -> a.getRegCode().equals(dmFilter.getRegionCode())).collect(Collectors.toList());	
				}else {
					resultList = circleList.stream().filter(a -> a.getRegCode().equals(dmFilter.getRegionCode()))
	...
	//SECTION REPORT
	@Transactional
	public void fetchYearWiseComparisonSections(String circleCode,String circleName) {
		Session session = null;
		try {
			session = sessionFactory.openSession();
			session.beginTransaction();
			...
			session.createNativeQuery(
					...
					.setParameter("complaintType", complaintType).setParameter("device", device)
					.setParameter("year1", year1).setParameter("month1", "")
					.setParameter("year2", year2).setParameter("month2", "")
					.executeUpdate();
			...
			FacesContext.getCurrentInstance().getExternalContext().redirect("yearWiseComparisonSectionReport.xhtml");
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("Error in database operation");
		}
	...
	//SECTION REPORT
		...
		public void fetchReportForSectionUsers() {
			...
			try {
				...
				String complaintType = dmFilter.getComplaintType()!=null ? dmFilter.getComplaintType() : "AL";
				String device = dmFilter.getDevice()!=null ? dmFilter.getDevice() : "L";
				String year1 = dmFilter.getYear1()!=null ? dmFilter.getYear1() : "2024";
	...
	public void redirectToComparsionCircleReport() throws IOException {
		FacesContext.getCurrentInstance().getExternalContext().redirect("yearWiseComparisonCircleReport.xhtml");
	}
	...
	public void getComplaintListForSection(String secCode,String sectionName) throws IOException {
		try (Session session = sessionFactory.openSession()) {
	        ...
	        if (dmFilter.getYear2() != null) {
	            ...
	            calendar.set(year2, Calendar.JANUARY, 1, 0, 0, 0);
	            fromDate2 = calendar.getTime();

	            calendar.set(year2, Calendar.DECEMBER, 31, 23, 59, 59);
			...
			System.out.println("FROM DATE 1=="+fromDateOne);
			System.out.println("TO DATE 1==="+toDateOne);
			System.out.println("THE SECTION ===="+secCode);
			
			System.out.println("***************************************************************************");
			
			System.out.println("FROM DATE 2=="+fromDateTwo);
			System.out.println("TO DATE 2==="+toDateTwo);
        ...
        FacesContext.getCurrentInstance().getExternalContext().redirect("yearWiseComparisonComplaintList.xhtml");
...
//            dto.setQcRemarks((String) row[28]);        	
	...
	public void exportYearWiseComparisonCircleToExcel(List<ComparisonReportValueBean> resultList) throws IOException {
	    HSSFWorkbook workbook = new HSSFWorkbook();
	    Sheet sheet = workbook.createSheet("YearWise_Comparison_Abstract_Report_CircleWise");
	    ...
	    // Create cell style for data cells with borders
	    ...
	    headingCell.setCellValue("YEAR WISE COMPARISON ABSTRACT REPORT - CIRCLE WISE");
	    ...
	    dateCell.setCellValue("YEAR 1: " + dmFilter.getYear1() +" | "+ " YEAR 2: " + dmFilter.getYear2());
	    ...
	    String period1Header = dmFilter.getYear1();
	    String period2Header = dmFilter.getYear2();
	    ...
	    for (ComparisonReportValueBean report : resultList) {
	        ...
	        snCell.setCellValue(String.valueOf(serialNumber++));
	        ...
	        setCellValueWithStyle(row, 2, report.getTot1(), dataCellStyle);
	        setCellValueWithStyle(row, 3, report.getCpl1(), dataCellStyle);
	        setCellValueWithStyle(row, 4, report.getPend1(), dataCellStyle);
	        setCellValueWithStyle(row, 5, report.getTot2(), dataCellStyle);
	        setCellValueWithStyle(row, 6, report.getCpl2(), dataCellStyle);
	        setCellValueWithStyle(row, 7, report.getPend2(), dataCellStyle);
	    ...
	    addTotalRowExcelForCircle(sheet, rowNum, totalSums, workbook);
	    ...
	    excelFile = DefaultStreamedContent.builder()
	            .name("YearWise_Comparison_Abstract_Report_CircleWise.xls")
	            .contentType("application/vnd.ms-excel")
	            .stream(() -> new ByteArrayInputStream(outputStream.toByteArray()))
	            .build();
	}
	private void addTotalRowExcelForCircle(Sheet sheet, int rowNum, BigDecimal[] totalSums, Workbook workbook) {
	    Row totalRow = sheet.createRow(rowNum);

	    CellStyle totalRowStyle = workbook.createCellStyle();
	    HSSFFont totalFont = ((HSSFWorkbook) workbook).createFont();
	    ...
	    for (int i = 0; i < totalSums.length; i++) {
	        Cell totalCell = totalRow.createCell(i + 2); // Start from column 3
	...
	public void exportYearWiseComparisonSectionsToExcel(List<ComparisonSectionValueBean> sectionList) throws IOException {
	    HSSFWorkbook workbook = new HSSFWorkbook();
	    Sheet sheet = workbook.createSheet("YearWise_Comparison_Abstract_Report_SectionWise");
	    ...
	    Row headingRow = sheet.createRow(0);
	    Cell headingCell = headingRow.createCell(0);
	    headingCell.setCellValue("YEAR WISE COMPARISON ABSTRACT REPORT - SECTION WISE");
	    headingCell.setCellStyle(headingStyle);
	    ...
	    dateCell.setCellValue("YEAR 1: " + dmFilter.getYear1() + " YEAR 2: " + dmFilter.getYear2() +"  CIRCLE :"+selectedCircleName);
	    dateCell.setCellStyle(dateStyle);
	    ...
	    for (ComparisonSectionValueBean report : sectionList) {
	        ...
	        setCellValueWithStyle(row, 3, report.getTot1(), dataCellStyle);
	        setCellValueWithStyle(row, 4, report.getCpl1(), dataCellStyle);
	        setCellValueWithStyle(row, 5, report.getPend1(), dataCellStyle);
	        setCellValueWithStyle(row, 6, report.getTot2(), dataCellStyle);
	        setCellValueWithStyle(row, 7, report.getCpl2(), dataCellStyle);
	        setCellValueWithStyle(row, 8, report.getPend2(), dataCellStyle);
	    ...
	    addTotalRowExcel(sheet, rowNum, totalSums, workbook);
	    ...
	    excelFile = DefaultStreamedContent.builder()
	            .name("YearWise_Comparison_Abstract_Report_SectionWise.xls")
	            .contentType("application/vnd.ms-excel")
	            .stream(() -> new ByteArrayInputStream(outputStream.toByteArray()))
	            .build();
	}

	private void addTotalRowExcel(Sheet sheet, int rowNum, BigDecimal[] totalSums, Workbook workbook) {
	    Row totalRow = sheet.createRow(rowNum);
	 ...
	 // Create cells for S.No, Section, Division columns
	    for (int i = 0; i < 3; i++) {
	        Cell cell = totalRow.createCell(i);
	        if (i == 0) {
	            cell.setCellValue("TOTAL");
	    ...
	    for (int i = 0; i < totalSums.length; i++) {
	        Cell totalCell = totalRow.createCell(i + 3); // Start from column 3
	        totalCell.setCellValue(totalSums[i].doubleValue());
	        totalCell.setCellStyle(totalRowStyle);
	    }
	}
	...
	public void exportYearWiseComparisonCircleToPDF(List<ComparisonReportValueBean> resultList) {
	    try {
	        ...
	        table.setWidths(new float[]{1.5f,2f, 1.5f, 1.5f, 1.5f, 1.5f, 1.5f, 1.5f}); 
	        ...
	        Paragraph title = new Paragraph("YEAR WISE COMPARISON CIRCLE REPORT",titleFont);
	        ...
	        Paragraph subTitle = new Paragraph("Complaint Type :"+complaintType+ "   Device :"+device, subTitleFont);
	        ...
	        pdfFile = DefaultStreamedContent.builder()
	                .name("YearWise_Comparison_Abstract_Report_Circle_Wise.pdf")
	...
	private void addTotalNumericCell(PdfPTable table, BigDecimal value) {
	    PdfPCell cell = new PdfPCell(new Phrase(value.toString(), FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10)));
	    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
	...
	private void addNumericCell(PdfPTable table, BigDecimal value) {
	    PdfPCell cell = new PdfPCell(new Phrase(value != null ? value.toString() : "0", FontFactory.getFont(FontFactory.HELVETICA, 10)));
	...
	public void exportYearWiseComparisonSectionsToPDF(List<ComparisonSectionValueBean> sectionList) {
	    try {
	        ByteArrayOutputStream baos = new ByteArrayOutputStream();
	        Document document = new Document(PageSize.A4.rotate());
	        PdfWriter.getInstance(document, baos);
	        ...
	        table.setSpacingBefore(10f);
	        table.setSpacingAfter(10f);
	        table.setWidths(new float[]{1.5f,2f,2f, 1.5f, 1.5f, 1.5f, 1.5f, 1.5f, 1.5f}); 
	        ...
	        Paragraph title = new Paragraph("YEAR WISE COMPARISON SECTION REPORT",titleFont);
	        title.setAlignment(Element.ALIGN_CENTER);
	        title.setSpacingAfter(5f);
	        ...
	        Font subtitleFont = new Font(Font.FontFamily.HELVETICA, 12, Font.BOLD, BaseColor.BLACK);
	        Paragraph circleParagraph = new Paragraph("Circle: " + selectedCircleName, subtitleFont);
	        circleParagraph.setAlignment(Element.ALIGN_CENTER);
	        circleParagraph.setSpacingAfter(10);
	        document.add(circleParagraph);
	        ...
	        pdfFile = DefaultStreamedContent.builder()
	                .name("YearWise_Comparison_Abstract_Report_Section_Wise.pdf")
	                .contentType("application/pdf")
	                .stream(() -> new ByteArrayInputStream(baos.toByteArray()))
	                .build();
	    } catch (DocumentException e) {
...
//COMPLAINT LIST TO EXCEL	
	  public void exportComplaintListToExcel(List<ViewComplaintReportValueBean> complaintList) throws IOException {
          ...
          ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
		    ...
		    try {
		        excelFile = DefaultStreamedContent.builder()
		                .name("Year_wise_Comparison_ComplaintList_Report.xls")
		                .contentType("application/vnd.ms-excel")
		                .stream(() -> inputStream)
		                .build();
		    } catch (Exception e) {
	  ...
	  public void exportComplaintListToPdf(List<ViewComplaintReportValueBean> complaintList) {
		  Document document = new Document(PageSize.A4.rotate());
		    ...
		    try {
		        ...
		        Paragraph title = new Paragraph("YEAR WISE COMPARISON COMPLAINT LIST", titleFont);
		        title.setAlignment(Element.ALIGN_CENTER);
		        ...
		        pdfFile = DefaultStreamedContent.builder()
		            .contentType("application/pdf")
		            .name("Year_wise_Comparison_ComplaintList_Report.pdf")
		            .stream(() -> new ByteArrayInputStream(outputStream.toByteArray()))
		            .build();
